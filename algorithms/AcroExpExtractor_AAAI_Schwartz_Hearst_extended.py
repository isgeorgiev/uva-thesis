...
class AcroExpExtractor_AAAI_Schwartz_Hearst_Extended(AcroExpExtractorRb):
    """Implementation of the extension of Schwartz and Hearts rule based algorithm."""
    ...
    def _find_best_long_form_extended(self, short_form, long_form):
        ...
        for s_index in range(len(short_form_capitals) - 1, -1, -1):
            ...
            if(r_index < 0):
                r_index = l_index
        if(l_index >= 0):
            # l_index = long_form_words.rfind(" ", 0, l_index + 1) + 1
    ...
    def _extra_rules(self, short_form, long_form):
        """ Extra rules to improve the accuracy of the algorithm """
        expansion = long_form
        ...
        # double check this rule
        # if(len(short_form_capital_letters) > len(first_letters_expansion)):
        # Check if the first letter of the expansion is the same as the letters of the short form
        ...
        # double check this rule, lower recall, much higher precision
        # if(len(first_letters_expansion_copy) + stopwords_expansion  < len(short_form_capital_letters)):
        #     return None;
        ...
        # if(capital_letters_expansion_words is not None and len(''.join(capital_letters_expansion_words)) > len(short_form_capital_letters)):
            return None
        ## end check this rule
    ...
    def get_acronyms_expansions_from_text(self, text):
        ...
        for paragraphs in text_split:
            for sentence in sent_tokenize(paragraphs):
                ...
                if( len(list(set(abbrev_map.keys())))> 0 and len(list(tokenized_text))>0) :
                    ...
                    for acronym_index, acronym in acronym_map.items():
                        # LENGTH_OF_ACRONYM = len(acronym.strip())
        ...
        for acronym, expansion in abbrev_map.items():
            if expansion is not None and (not acronym in final_map.keys() or final_map[acronym] is None or final_map[acronym] == ""):
                final_map[acronym] = expansion

        return final_map

if __name__ == "__main__":
    acroExp = AcroExpExtractor_AAAI_Schwartz_Hearst_Extended()
